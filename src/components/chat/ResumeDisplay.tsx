...
const ResumeDisplay = ({ content, isStreaming }: Props) => {
  ...
  return (
    <div className="mt-10">
      {/* Header Badge */}
      {/* Header Badge */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-3 border-b border-primary/10">
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-4 w-full sm:w-auto">
          {/* Animated Tabs */}
          <div className="flex flex-1 sm:flex-none items-center gap-1 p-1 border border-border rounded-lg bg-background/50">
            <button
              onClick={() => setActiveMode('preview')}
              className={cn(
                'relative flex-1 sm:flex-none flex justify-center items-center gap-2 px-3 sm:px-4 py-1 text-sm font-medium rounded-md transition-colors z-10 cursor-pointer',
                activeMode === 'preview'
            ...
            >
              ...
              <span className="md:inline">Preview</span>
              {activeMode === 'preview' && (
                <motion.div
            ...
            <button
              ...
              className={cn(
                'relative flex-1 sm:flex-none flex justify-center items-center gap-2 px-3 sm:px-4 py-1 text-sm font-medium rounded-md transition-colors z-10 cursor-pointer disabled:cursor-not-allowed',
                activeMode === 'edit'
                  ? 'text-primary-foreground'
                  : 'text-muted-foreground hover:text-foreground'
              )}
          ...
          <Button
            ...
            className="gap-2 border-border hover:bg-secondary/50 cursor-pointer disabled:cursor-not-allowed px-3 sm:px-4"
          >
            {isExporting ? (
              <>
