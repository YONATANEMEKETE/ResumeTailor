...
export function MorphSurface({
  ...
  return (
    <div
      className={cn('flex justify-center items-end z-50', className)}
      style={{
        width: showFeedback
          ? hookExpandedWidth
          : collapsedWidth === 'auto'
    ...
    >
      <motion.div
        ...
        animate={{
          ...
          height: showFeedback ? hookExpandedHeight : hookCollapsedHeight,
          borderRadius: showFeedback ? 12 : 8,
        }}
        transition={
          springConfig || {
            type: 'spring',
...
function MorphSurfaceDock() {
  ...
  const defaultTrigger = (
    <button
      type="button"
      className={cn(
        'flex justify-center items-center rounded-md p-2 flex-1 gap-2 w-full',
  ...
  return (
    <footer className="flex items-center justify-center select-none whitespace-nowrap mt-auto h-[44px] w-full">
      <div className="flex items-center justify-center gap-6 px-2 w-full">
...
>(({ onSuccess }, ref) => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    closeFeedback(); // Collapse immediately
